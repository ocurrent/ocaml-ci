...
ADD --chown=opam . .
RUN opam config exec -- dune build ./_build/install/default/bin/opam-ci
...
ENTRYPOINT ["dumb-init", "/usr/local/bin/opam-ci"]
COPY --from=build /src/_build/install/default/bin/opam-ci /usr/local/bin/
