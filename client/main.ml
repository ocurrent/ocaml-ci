...
let errorf msg = msg |> Fmt.kstrf @@ fun msg -> Error (`Msg msg)
...
let show_status job =
  ...
  |> Lwt_result.map @@ fun { Current_rpc.Job.id; description; can_cancel; can_rebuild } ->
     Fmt.pr "@[<v2>Job %S:@,Description: @[%a@]@,Can cancel: %b@,Can rebuild: %b@]@." id
       Fmt.lines description can_cancel can_rebuild
...
let list_orgs ci =
  ...
  |> Lwt_result.map @@ function
     ...
     | orgs ->
         Fmt.pr "@[<v>No owner (organisation) given. Try one of these:@,@,%a@]@."
           Fmt.(list string)
           orgs
  ...
  let parse s =
    if not (Stdlib.String.contains s '/') then
      if String.length s < 6 then
        Error
          (`Msg
            "Git reference should start 'refs/' or be a hash at least 6 characters long")
      else Ok (`Commit s)
    else if String.is_prefix ~affix:"refs/" s then make_ref s
    else make_ref ("refs/" ^ s)
  in
  let pp f = function `Commit s -> Fmt.string f s | `Ref r -> Fmt.string f r in
  Arg.conv (parse, pp)
...
let cmd =
  ...
  let main ci_uri repo target variant job_op =
    ...
    | Error (`Msg m) ->
        ...
        exit 1
  in
  (Term.(const main $ cap $ repo $ target $ variant $ job_op), Term.info "ocaml-ci" ~doc)
