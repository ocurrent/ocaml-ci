let install_ocamlformat =
  ...
  | Opam { version } -> [run ~network ~cache "opam depext -i ocamlformat=%s" version]
...
let fmt_spec ~base ~ocamlformat_source =
  ...
  @ (match ocamlformat_source with Some src -> install_ocamlformat src | None -> [])
  @ [
      copy ["."] ~dst:"/src/";
...
let doc_spec ~base ~opam_files ~selection =
  ...
  @@ comment "%s" (Fmt.strf "%a" Variant.pp selection.Selection.variant)
     :: user ~uid:1000 ~gid:1000 :: Opam_build.install_project_deps ~opam_files ~selection
  @ [
      ...
      copy ["."] ~dst:"/src/";
      run
        "ODOC_WARN_ERROR=true opam exec -- dune build @doc || (echo \"dune build @doc \
         failed\"; exit 2)";
    ]
...
let install_opam_dune_lint ~cache ~network ~base =
  ...
  stage ~from:base
    [
      user ~uid:1000 ~gid:1000;
      run ~cache ~network
        "git -C ~/opam-repository pull origin master && opam update && opam pin add -yn \
         opam-dune-lint.dev \
         https://github.com/ocurrent/opam-dune-lint.git#a431a1128b9564d1b3d08b8bee2977be47bc6b6b";
