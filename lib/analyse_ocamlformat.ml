...
let ocamlformat_version_from_string =
  ...
  in
  fun path ->
    Re.exec_opt re path |> function Some g -> Some (Re.Group.get g 1) | None -> None
...
let ocamlformat_version_from_file ~root job path =
  ...
  if not (Sys.file_exists path) then
    ...
    match versions with
    | [v] ->
        let () = Current.Job.log job "Found OCamlformat version '%s' in dotfile" v in
    ...
    | [] -> (
        ...
        in
        Current.Process.check_output ~cwd:root ~cancellable:true ~job cmd >>!= function
        | "" ->
            if List.mem "disable" lines then Lwt.return (Ok None)
            else Lwt.return (Error (`Msg "Missing 'version=' line in .ocamlformat"))
        | _ ->
            Lwt.return
              (Error
                 (`Msg
                   "Missing 'version=' line in .ocamlformat (disabled, but some \
                    .ocamlformat-enable files are present)")))
    | _ :: _ :: _ -> Lwt.return (Error (`Msg "Multiple 'version=' lines in .ocamlformat"))
