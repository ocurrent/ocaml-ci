...
module TimeClient = struct
  let src =
    Logs.Src.create "ocaml_ci.run_time.lib_client"
      ~doc:"ocaml-ci lib-client run-time"
          ...
          Option.fold ji.queued_at
            ...
            ~some:(fun q ->
              Option.fold ji.finished_at
                ~none:
                  (Error "Finished_at is None - cannot construct timestamp.")
    ...
    let of_timestamp ~build_created_at ?(current_time = Unix.gettimeofday ())
      ...
      | Finished { queued_at; started_at; finished_at } -> (
          if finished_at < build_created_at then Cached
            ...
            | None ->
                Finished
                  { queued_for = finished_at -. queued_at; ran_for = None }
            | Some v ->
                Finished
                  {
...
module Job = struct
  ...
  let total_of_run_times (build : Client.job_info list) =
    ...
    |> List.fold_left
         (fun subtotal rt -> subtotal +. TimeClient.TimeInfo.total rt)
         0.
