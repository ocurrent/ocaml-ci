...
let maybe_add_dev ~dir name =
  if Fpath.is_current_dir dir || not (String.contains (Fpath.basename dir) '.') then
    name ^ ".dev"
  else name
...
let pin_opam_files ~network groups =
  if groups = [] then []
    ...
    mkdir groups
    @ (groups |> List.map (fun (dir, files, _) -> copy files ~dst:(Fpath.to_string dir)))
    @ [
        groups
...
let install_project_deps ~opam_files ~selection =
  ...
  let open Obuilder_spec in
  let cache =
    [Obuilder_spec.Cache.v download_cache ~target:"/home/opam/.opam/download-cache"]
  in
  let network = ["host"] in
  let distro_extras =
  ...
  @ [
      ...
      run ~network ~cache
        "cd ~/opam-repository && (git cat-file -e %s || git fetch origin master) && git \
         reset -q --hard %s && git log --no-decorate -n1 --oneline && opam update -u"
        commit commit;
    ]
...
let spec ~base ~opam_files ~selection =
  ...
  stage ~from:base
    (comment "%s" (Fmt.strf "%a" Variant.pp selection.Selection.variant)
     :: user ~uid:1000 ~gid:1000 :: install_project_deps ~opam_files ~selection
    @ [
        copy ["."] ~dst:"/src/";
