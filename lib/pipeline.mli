...
val build_with_docker :
  ...
  (string * ([> `Built | `Checked ] Current_term.Output.t * string option))
    list Current.t
(** [build_with_docker ~repo ~analysis ~platforms commit] creates a suite of builds to perform against [commit].

     The builds created will depend on the [platforms] available and the solver [analysis] for those platforms. Optionally the builds can be peformed locally in docker or on an OCluster instance. *)