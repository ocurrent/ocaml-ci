...
CREATE TABLE IF NOT EXISTS ci_build_index (
  ...
  PRIMARY KEY (owner, name, hash, variant)
     ...
     let record_job =
       Sqlite3.prepare db
         "INSERT OR REPLACE INTO ci_build_index (owner, name, hash, variant, job_id) \
          VALUES (?, ?, ?, ?, ?)"
     in
     let remove =
       Sqlite3.prepare db
         "DELETE FROM ci_build_index WHERE owner = ? AND name = ? AND hash = ? AND \
          variant = ?"
     in
     let get_jobs =
       Sqlite3.prepare db
         "SELECT ci_build_index.variant, ci_build_index.job_id, cache.ok, cache.outcome \
          FROM ci_build_index LEFT JOIN cache ON ci_build_index.job_id = cache.job_id \
          WHERE ci_build_index.owner = ? AND ci_build_index.name = ? AND \
          ci_build_index.hash = ?"
     in
     let get_job =
       Sqlite3.prepare db
         "SELECT job_id FROM ci_build_index WHERE owner = ? AND name = ? AND hash = ? \
          AND variant = ?"
     in
     let get_job_ids =
       Sqlite3.prepare db
         "SELECT variant, job_id FROM ci_build_index WHERE owner = ? AND name = ? AND \
          hash = ?"
     in
     let full_hash =
       Sqlite3.prepare db
         "SELECT DISTINCT hash FROM ci_build_index WHERE owner = ? AND name = ? AND hash \
          LIKE ?"
     in
     { db; record_job; remove; get_jobs; get_job; get_job_ids; full_hash })
    ...
    let set job_id =
      Log.info (fun f ->
          f "@[<h>Index.record %s/%s %s %s -> %a@]" owner name
            ...
            variant
            Fmt.(option ~none:(unit "-") string)
            job_id);
      match job_id with
      | None ->
          Db.exec t.record_job
            Sqlite3.Data.[TEXT owner; TEXT name; TEXT hash; TEXT variant; NULL]
      | Some id ->
          Db.exec t.record_job
            Sqlite3.Data.[TEXT owner; TEXT name; TEXT hash; TEXT variant; TEXT id]
    in
    let update j1 j2 =
      match (j1, j2) with
...
let get_full_hash ~owner ~name short_hash =
  ...
  if is_valid_hash short_hash then
    match
      Db.query t.full_hash Sqlite3.Data.[TEXT owner; TEXT name; TEXT (short_hash ^ "%")]
    with
    | [] -> Error `Unknown
...
let get_job ~owner ~name ~hash ~variant =
  let t = Lazy.force db in
  match
    Db.query_some t.get_job Sqlite3.Data.[TEXT owner; TEXT name; TEXT hash; TEXT variant]
  with
...
let set_active_refs ~repo refs = active_refs := Repo_map.add repo refs !active_refs
