...
module Op = struct
  ...
  let run { Builder.docker_context; pool; build_timeout } job
    ...
    Current.Job.write job
      (Fmt.strf "@[<v>Base: %a@,%a@]@." Raw.Image.pp base Spec.pp_summary ty);
    Current.Job.write job
      (Fmt.strf
         "@.To reproduce locally:@.@.%a@.cat > Dockerfile \
          <<'END-OF-DOCKERFILE'@.\o033[34m%s\o033[0mEND-OF-DOCKERFILE@.docker build \
          .@.@."
    ...
    Current.Job.start ~timeout:build_timeout ~pool job ~level:Current.Level.Average
    >>= fun () ->
  ...
  let pp f ({ Key.repo; commit; label }, _) =
    Fmt.pf f "test %a %a (%s)" Current_github.Repo_id.pp repo Current_git.Commit.pp commit
      label
