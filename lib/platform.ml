...
let compiler_matches_major_and_minor vars ~version =
  ...
  in
  Ocaml_version.equal vars_version (Ocaml_version.with_just_major_and_minor version)
...
module Query = struct
  ...
  module Outcome = struct
    ...
    let unmarshal s =
      match of_yojson (Yojson.Safe.from_string s) with Ok x -> x | Error e -> failwith e
  end
  ...
  let run No_context job { Key.docker_context; variant } { Value.image; host_image } =
    ...
    let arch =
      ...
      if Ocaml_version.arch_is_32bit v then Some (Ocaml_version.to_opam_arch v) else None
    in
      ...
      | `Assoc items ->
          `Assoc
            (("ocaml_package", `String ocaml_package_name)
             :: ("ocaml_version", `String ocaml_version) :: items)
      | json ->
          Fmt.failwith "Unexpected JSON: %a" Yojson.Safe.(pretty_print ~std:true) json
    in
