...
let is_empty_file x =
  match Unix.lstat x with Unix.{ st_kind = S_REG; st_size = 0; _ } -> true | _ -> false
...
module Analysis = struct
  type t = {
    ...
    selections :
      [ `Opam_build of Selection.t list | `Opam_monorepo of Opam_monorepo.config ];
  }
  [@@deriving yojson]
  ...
  let check_opam_version =
    ...
    fun name opam ->
      ...
      if OpamVersion.compare opam_version version_2 < 0 then
        Fmt.failwith "Package %S uses unsupported opam version %s (need >= 2)" name
  ...
  let solve ~root_pkgs ~pinned_pkgs ~platforms ~opam_repository_commit ~job ~solver =
    let platforms =
      List.map (fun (variant, vars) -> (Variant.to_string variant, vars)) platforms
    in
    let request =
      {
...
module Examine = struct
  ...
  module Value = struct
    ...
    let platform_to_yojson (variant, vars) =
      `Assoc
        [("variant", Variant.to_yojson variant); ("vars", Worker.Vars.to_yojson vars)]
