...
let run_time_info =
  let state f (rt : Run_time.run_time_info) = Fmt.pf f "%a" Run_time.pp_run_time_info rt in
  let equal (rt1 : Run_time.run_time_info) (rt2 : Run_time.run_time_info) =
    match (rt1, rt2) with
    | Cached, Cached -> true
    | Queued_for v1, Queued_for v2 -> cmp_floats v1 v2
    | Running v1, Running v2 -> cmp_floats v1.queued_for v2.queued_for && cmp_floats v1.ran_for v2.ran_for
    | Finished v1, Finished v2 ->
      (match v1.ran_for, v2.ran_for with
      | None, None -> cmp_floats v1.queued_for v2.queued_for
      | Some r1, Some r2 -> cmp_floats v1.queued_for v2.queued_for && cmp_floats r1 r2
...
let test_running () =
  Job.timestamp := (fun () -> 0.1); (* Note that this is what is used for the starting-at timestamp *)
  ...
  Current.Db.exec_literal db
    (Fmt.str "INSERT INTO cache (op, key, job_id, value, ok, outcome, ready, running, \
     finished, build) \n\
     VALUES ('test42', x'00', '%s', x'01', 1, x'02', '1970-01-01 00:00', \
     '1970-01-01 00:01', '1970-01-01 00:00', 0)" (Job.id job1));
  let expected : Run_time.timestamps = Run_time.Running {ready=0.; started=0.1} in
...
let test_simple_run_times () =
  ...
  let expected : Run_time.timestamps = Finished { ready=1572598800.;
...
let test_info_from_timestamps_queued () =
  ...
  Alcotest.(check run_time_info) "info_from_timestamps - queued" [expected] [result]
...
let test_info_from_timestamps_running () =
  ...
  let result = Run_time.info_from_timestamps ~current_time st in
  Alcotest.(check run_time_info) "info_from_timestamps - running" [expected] [result]

let test_info_from_timestamps_cached () =
  let current_time = 100. in
  let st : Run_time.timestamps = Finished {ready=42.; started=Some 60.; finished=82.} in
  let expected = Run_time.Cached (* finished < current_time *) in
  let result = Run_time.info_from_timestamps ~current_time st in
  Alcotest.(check run_time_info) "info_from_timestamps - finished" [expected] [result]

let test_info_from_timestamps_finished () =
  let current_time = 100. in
  let st : Run_time.timestamps = Finished {ready=42.; started=Some 60.; finished=182.} in
...
let test_info_from_timestamps_never_ran () =
  let current_time = 100. in
  let st : Run_time.timestamps = Finished {ready=42.; started=None; finished=182.} in
...
let test_merge_queued_finished () =
  let st1 : Run_time.timestamps = Queued 12. in
  let st2 : Run_time.timestamps = Finished {ready=22.; started=Some 40.; finished=55.} in
...
let test_merge_running_finished () =
  ...
  let st2 : Run_time.timestamps = Finished {ready=22.; started=None; finished=55.} in
  let expected : Run_time.timestamps = Running {ready=12.; started=42.} in
  let result = Run_time.merge st1 st2 in
  Alcotest.(check timestamps) "merge_running_finished" [expected] [result]

let test_merge_finished_finished () =
  let st1 : Run_time.timestamps = Finished {ready=12.; started=Some 40.; finished=55.} in
  let st2 : Run_time.timestamps = Finished {ready=22.; started=Some 30.; finished=45.} in
  let expected : Run_time.timestamps = Finished {ready=12.; started=Some 30.; finished=55.} in
  let result = Run_time.merge st1 st2 in
  Alcotest.(check timestamps) "merge_finished_finished" [expected] [result];

  let st2 : Run_time.timestamps = Finished {ready=22.; started=None; finished=45.} in
  (* Since the second step never ran, the collection [st1, st2] did not run *)
  let expected : Run_time.timestamps = Finished {ready=12.; started=None; finished=55.} in
  let result = Run_time.merge st1 st2 in
  Alcotest.(check timestamps) "merge_finished_finished" [expected] [result]
...
let test_timestamps_of_build () =
  ...
  Index.record ~repo ~hash ~status:`Passed ~gref:"master" [ ("analysis", Some "job1"); ("alpine", Some "job2") ];
...
let test_timestamps_of_three_steps_build () =
  ...
  Index.record ~repo ~hash ~status:`Passed ~gref:"master" [ ("analysis", Some "job1");
                                                            ("alpine", Some "job2");
                                                            ("debian", Some "job3")];
...
let tests =
  [
    ...
    Alcotest_lwt.test_case_sync "info_from_timestamps_queued" `Quick test_info_from_timestamps_queued;
    Alcotest_lwt.test_case_sync "info_from_timestamps_running" `Quick test_info_from_timestamps_running;
    Alcotest_lwt.test_case_sync "info_from_timestamps_finished" `Quick test_info_from_timestamps_finished;
    Alcotest_lwt.test_case_sync "info_from_timestamps_cached" `Quick test_info_from_timestamps_cached;
    Alcotest_lwt.test_case_sync "info_from_timestamps_never_ran" `Quick test_info_from_timestamps_never_ran;
    Alcotest_lwt.test_case_sync "merge_queued_queued" `Quick test_merge_queued_queued;
    Alcotest_lwt.test_case_sync "merge_queued_running" `Quick test_merge_queued_running;
    Alcotest_lwt.test_case_sync "merge_queued_finished" `Quick test_merge_queued_finished;
    Alcotest_lwt.test_case_sync "merge_running_running" `Quick test_merge_running_running;
    Alcotest_lwt.test_case_sync "merge_running_finished" `Quick test_merge_running_finished;
    Alcotest_lwt.test_case_sync "merge_finished_finished" `Quick test_merge_finished_finished;
    Alcotest_lwt.test_case_sync "timestamps_of_build" `Quick test_timestamps_of_build;
    Alcotest_lwt.test_case_sync "timestamps_of_queued_build" `Quick test_timestamps_of_queued_build;
    Alcotest_lwt.test_case_sync "timestamps_of_queued_build" `Quick test_timestamps_of_three_steps_build;
  ]
