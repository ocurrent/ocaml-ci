...
let test_simple () =
  ...
  Current.Db.exec_literal db
    "INSERT INTO cache (op, key, job_id, value, ok, outcome, ready, running, finished, \
     build)\n\
    \                                     VALUES ('test', x'00', 'job1', x'01', 1, \
     x'02', '2019-11-01 9:00', '2019-11-01 9:01', '2019-11-01 9:02', 0)";
  ...
  Alcotest.(check jobs) "Jobs" [("alpine", `Not_started); ("analysis", `Passed)]
  @@ Index.get_jobs ~owner ~name hash;
  Current.Db.exec_literal db
    "INSERT INTO cache (op, key, job_id, value, ok, outcome, ready, running, finished, \
     build)\n\
    \                                     VALUES ('test', x'01', 'job2', x'01', 0, \
     x'21', '2019-11-01 9:03', '2019-11-01 9:04', '2019-11-01 9:05', 0)";
  Index.record ~repo ~hash ~status:`Failed
    [("analysis", Some "job1"); ("alpine", Some "job2")];
  Alcotest.(check jobs) "Jobs" [("alpine", `Failed "!"); ("analysis", `Passed)]
  @@ Index.get_jobs ~owner ~name hash;
