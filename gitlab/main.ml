...
let main () config mode app capnp_public_address capnp_listen_address gitlab_auth submission_uri : ('a, [`Msg of string]) result =
  Lwt_main.run begin
...
let cmd =
  ...
  Cmd.v info 
    Term.(term_result (const main $ setup_log $ Current.Config.cmdliner 
                       $ Current_web.cmdliner $ Current_gitlab.Api.cmdliner 
                       $ capnp_public_address $ capnp_listen_address
                       $ Current_gitlab.Auth.cmdliner $ submission_service ))
