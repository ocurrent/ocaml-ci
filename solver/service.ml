...
module Epoch : sig
  ...
  val handle :
    log:Ocaml_ci_api.Solver.Log.t -> Worker.Solve_request.t -> t -> Selection.t list Lwt.t
  ...
  let validate (worker : Lwt_process.process) =
    ...
    | Lwt.Return status ->
        Format.eprintf "Worker %d is dead (%a) - removing from pool@." worker#pid
  ...
  let create ~n_workers ~create_worker hash =
    ( Opam_repository.open_store () >>= fun store ->
      ...
      | false -> (
          Fmt.pr "Need to update opam-repository to get new commit %a@." Store.Hash.pp
            hash;
  ...
  let process ~log ~id request worker =
    ...
    | Some len -> (
        ...
        | '+' ->
            ...
            let packages =
              Astring.String.with_range ~first:1 results |> Astring.String.cuts ~sep:" "
            in
            Ok packages
        | '-' ->
  ...
  let handle ~log request t =
    let { Worker.Solve_request.opam_repository_commit; platforms; root_pkgs; pinned_pkgs }
        =
      request
    in
           ...
           | Ok packages ->
               let repo_packages =
                 ...
                 |> List.filter_map (fun pkg ->
                        ...
                        if OpamPackage.Name.Set.mem pkg.name pins then None else Some pkg)
               in
       ...
       method solve_impl params release_param_caps =
         ...
         | Some log -> (
             ...
             | Ok request ->
                 ...
                 >|= fun selections ->
                 let json =
                   Yojson.Safe.to_string (Worker.Solve_response.to_yojson selections)
                 in
