    ...
    | Finished of { queued_for : float; ran_for : float option }
        (** Type to encode durations. All floats represent seconds.
            ...
            - Cached - a previously run instance of the step was used and
              nothing was run
            - Queued - the associated duration represents the number of seconds
              it has been queued for (relative to now)
...
val queued_for : TimeInfo.t -> float
val ran_for : TimeInfo.t -> float

val eq_timestamps : Timestamp.t -> Timestamp.t -> bool
(** equality of timestamps - useful for tests *)

val run_times_from_timestamps :
  build_created_at:float -> ?current_time:float -> Timestamp.t -> TimeInfo.t
...
val timestamps_from_job_info :
  Ocaml_ci_api.Client.job_info -> (Timestamp.t, string) result
...
val total_time : TimeInfo.t -> float
