...
let has_role user = function
  ...
  | _ -> (
      ...
      | Some ("github:talex5" | "github:avsm" | "github:kit-ty-kate" | "github:samoht") ->
          true
      | _ -> false)
...
let main config mode app capnp_address github_auth submission_uri :
  ...
  Lwt_main.run
    ( run_capnp capnp_address >>= fun (vat, rpc_engine_resolver) ->
      ...
      rpc_engine_resolver
      |> Option.iter (fun r -> Capability.resolve_ok r (Api_impl.make_ci ~engine));
...
let submission_service =
  ...
  @@ Arg.info ~doc:"The submission.cap file for the build scheduler service" ~docv:"FILE"
       ["submission-service"]
