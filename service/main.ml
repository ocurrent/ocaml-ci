...
let main () config mode app capnp_public_address capnp_listen_address
    github_auth submission_uri solve_uri migrations : ('a, [ `Msg of string ]) result =
  Lwt_main.run
...
let submission_solver_service =
  ...
  @@ Arg.info
       ~doc:
         "The solve.cap/submission.cap file for the build scheduler service \
          when a solver-worker is connected to that scheduler."
       ~docv:"FILE"
