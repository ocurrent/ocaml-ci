...
let ( >>!= ) x f =
  x >>= function
  | Error (`Capnp ex) ->
      respond_error `Internal_server_error (Fmt.to_to_string Capnp_rpc.Error.pp ex)
  | Ok y -> f y
           ...
           |> List.map @@ fun r ->
              ...
              | "refs" :: "heads" :: branch ->
                  ...
                  span
                    [
                      txt "branch ";
                      a ~a:[a_href (github_branch_url ~owner ~name branch)] [txt branch];
                    ]
              | ["refs"; "pull"; id; "head"] ->
                  span
                    [
                      txt "PR ";
                      a ~a:[a_href (github_pr_url ~owner ~name id)] [txt ("#" ^ id)];
                    ]
...
let stream_logs job ~owner ~name ~refs ~hash ~jobs ~variant ~status (data, next) writer =
  ...
  let rec aux next =
    ...
    | Ok (data, next) ->
        Transfer_IO.write writer (Current_ansi.process ansi data) >>= fun () -> aux next
    | Error (`Capnp ex) ->
...
let repo_handle ~meth ~owner ~name ~repo path =
  ...
  | `GET, ["commit"; hash; "variant"; variant] ->
      ...
      in
      let res =
        Cohttp.Response.make ~status:`OK ~flush:true ~encoding:Cohttp.Transfer.Chunked
          ~headers ()
      in
      let write _ic oc =
        ...
        Lwt.finalize
          (fun () ->
            stream_logs job ~owner ~name ~refs ~hash ~jobs ~variant ~status chunk writer
...
let list_orgs ci =
  ...
  let body =
    Template.instance Tyxml.Html.[breadcrumbs [] "github"; ul (List.map format_org orgs)]
  in
  Server.respond_string ~status:`OK ~body () |> normal_response
