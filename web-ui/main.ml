...
let pp_mode f mode = Sexplib.Sexp.pp_hum f (Conduit_lwt_unix.sexp_of_server mode)
       ...
       Lwt.try_bind
         ...
         (function
           | Unix.Unix_error (Unix.EADDRINUSE, "bind", _) ->
               Fmt.failwith
                 "Web-server failed.@ Another program is already using this port %a."
                 pp_mode mode
           | ex -> Lwt.fail ex)
     in
...
let backend_cap =
  ...
  @@ Arg.info ~doc:"The capability file giving access to the CI backend service."
       ~docv:"CAP" ["backend"]
