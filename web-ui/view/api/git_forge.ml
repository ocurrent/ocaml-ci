...
(* This module is starting out as very thin wrapper on a string. It's
   ...
   different git-forges via a functorial approach. If the implementation
   between the different git-forges turns out to be the same, this module
   will not grow and we may choose to back away from this approach. *)
