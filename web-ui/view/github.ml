...
let list_orgs ~orgs =
  let org_table =
    let f { Client.CI.owner; description; n_repos } =
      let github_org_url org = Printf.sprintf "https://github.com/%s" org in
      ...
      let image =
        img
          ...
          ~alt:(Printf.sprintf "%s %s profile picture" owner prefix)
          ()
      in
      Org.row ~prefix ~owner ~description ~n_repos ~image
        ~url:(github_org_url owner)
    in
    List.map f orgs
  in
  let title =
  ...
  Template_v1.instance
    [
      ...
      div
        ~a:[ a_class [ "mt-8" ] ]
        [ div ~a:[ a_class [ "flex flex-col space-y-6" ] ] org_table ];
    ]
